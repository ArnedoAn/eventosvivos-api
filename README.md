...
The deployable unit is the container image built by the `Dockerfile`.

1. Build the image:

```bash
docker build -t eventosvivos-api .
```

2. Run it anywhere that hosts containers, supplying the required environment variables:
...
| `Reservation__PendingHoldsInventory` | Feature flag (see above) |
| `Reservation__PendingExpirationMinutes` | Feature flag (see above) |

3. Point the connection string at a managed PostgreSQL service (e.g., Neon, Supabase, Azure Database for PostgreSQL, Amazon RDS, Google Cloud SQL).

### Examples of interchangeable hosting options

These are examples only; the image itself has no provider coupling:

- **Render** (Web Service + managed Postgres)
- **Fly.io** (container app + Fly Postgres or external Postgres)
- **Azure Container Apps** (container app + Azure Database for PostgreSQL)
- **Railway** (container + Neon/Supabase Postgres)

The API applies migrations on startup, so a new deployment only needs the database reachable and the environment variables set.
